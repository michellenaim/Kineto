...
class Genre extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (
      this.props.genreId !== prevProps.genreId &&
      this.props.genreId !== 'users'
    ) {
      this.props.fetchGenre(this.props.genreId);
    }
  }
  ...
  render() {
    ...
    if (
      this.props.moviesgenres &&
      Object.values(this.props.moviesgenres).length < 1
    ) {
      return null;
    }
    ...
    return (
      <div className="genre">
        ...
        <div className="btn prev" onClick={() => this.scroll('left')}>
          {'<'}
        </div>
        ...
        <div className="btn next" onClick={() => this.scroll('right')}>
          {'>'}
        </div>
      </div>
    );
